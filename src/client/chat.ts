...
/** Encrypt a plaintext message for `peer`. */
export async function encryptFor(
	identity: Identity,
	peer: string,
	message: string,
): Promise<EncryptedPayload> {
	return encrypt(await conversationKey(identity, peer), message);
}

/** Decrypt a message that came from `peer`. Throws if it wasn't sealed for this
 * conversation (wrong peer, tampered ciphertext). */
export async function decryptFrom(
	identity: Identity,
	peer: string,
	payload: EncryptedPayload,
): Promise<string> {
	return decrypt(await conversationKey(identity, peer), payload);
}
